...
public class PatientItemRenderer implements ListitemRenderer<Patient>
{
    ...
    */
   public PatientItemRenderer(final boolean show){
   ...
   public void render(final Listitem li, final Patient data, final int index){
      ...
      if(showMedecin){
         drawMedecinsLabel(ManagerLocator.getPatientManager().getMedecinsManager(pat), li);
      }
   ...
   private void drawPrelevementsLabelWithPopup(final Listitem li, final Patient pat){
      ...
      for(int i = 0; i < maladies.size(); i++){
         final Iterator<Prelevement> it = ManagerLocator.getMaladieManager().getPrelevementsManager(maladies.get(i)).iterator();
   ...
   public void setFusion(final boolean is){
      this.isFusion = is;
   }
    ...
    */
   public static void drawMedecinsLabel(final List<Collaborateur> meds, final Listitem li){
